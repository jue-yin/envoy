...
void Filter::log(const Http::RequestHeaderMap* headers,
                 const Http::ResponseHeaderMap* responseHeaders,
                 const Http::ResponseTrailerMap* responseTrailers, const StreamInfo::StreamInfo&,
  ...
  case Envoy::AccessLog::AccessLogType::DownstreamEnd:
    ...
    if (initRequest()) {
      request_headers_ = const_cast<Http::RequestHeaderMap*>(headers);
    }
    ...
    activation_response_headers_ = responseHeaders;
    ...
    activation_response_trailers_ = responseTrailers;
    ...
    req_->is_golang_processing_log = 1;
    dynamic_lib_->envoyGoFilterOnHttpLog(req_, int(type), decoding_state, encoding_state,
                                         req_header_num, req_header_bytes, req_trailer_num,
                                         req_trailer_bytes, resp_header_num, resp_header_bytes,
                                         resp_trailer_num, resp_trailer_bytes);
    req_->is_golang_processing_log = 0;
    break;
  default:
...
CAPIStatus Filter::getStringValue(int id, uint64_t* value_data, int* value_len) {
  ...
  case EnvoyValue::FilterChainName:
    req_->strValue = streamInfo().filterChainName();
    break;
...
FilterConfig::FilterConfig(
    ...
    : plugin_name_(proto_config.plugin_name()), so_id_(proto_config.library_id()),
      ...
      concurrency_(context.getServerFactoryContext().options().concurrency()),
