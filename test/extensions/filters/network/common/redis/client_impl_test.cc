...
public:
  ...
  void testInitializeReadPolicy(
    ...
    std::string_view raw_readonly_request = Utility::makeRawReadOnlyRequest();
    EXPECT_CALL(*encoder_, encode(raw_readonly_request, _));
    EXPECT_CALL(*flush_timer_, enabled()).WillOnce(Return(false));
    client_->initialize(auth_username_, auth_password_, params_);
...
TEST(RedisRawClientFactoryImplTest, Basic) {
  ...
  RawClientPtr client = factory.create(host, dispatcher, config, redis_command_stats,
                                       *stats_.rootScope(), auth_username, auth_password, params);
  client->close();
}
...
TEST_F(RedisRawClientImplTest, CommandStatsDisableRequest) {
  ...
  std::string_view request1{request1_str};
  MockRawClientCallbacks callbacks1;
